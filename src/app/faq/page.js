    ...
    questions: [
      {
        ...
        a: "Yes. You must claim the pension while physically in Australia — you need to be present on the day you lodge your claim. Once granted, for most long-term Australians there is no mandatory waiting period before you can move overseas. Your pension is paid to you in Thailand indefinitely. From 20 September 2026, after 12 weeks abroad the Pension Supplement stops entirely. After 26 weeks your rate may be recalculated based on your years of Australian residency. If moving permanently, the supplement stops immediately on departure. See our [full pension guide](/blog/australian-pension-thailand) for the detail, and always confirm your specific situation with Centrelink International Services on 131 673 before making plans.",
      },
      {
...
// Convert markdown link syntax [text](url) to clickable HTML anchors.
// External links (http://...) open in a new tab; internal links stay in the same tab.
function renderAnswerHTML(text) {
  return text.replace(
    ...
    (_, label, url) => {
      ...
      return `<a href="${url}" style="color:#C9963A;text-decoration:underline"${target}>${label}</a>`;
    }
  );
}
                  ...
                  <details key={i} style={{ ...card, overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '0 20px 16px', fontSize: '14px', lineHeight: 1.75, color: '#7A6040', borderTop: '1px solid rgba(201,150,58,0.12)', paddingTop: '14px', marginTop: '0' }}>
                      <p
                        ...
                        dangerouslySetInnerHTML={{ __html: renderAnswerHTML(item.a) }}
                      />
                    </div>
                  </details>
                ))}
